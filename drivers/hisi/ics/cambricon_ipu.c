...
#define WATCHDOG_TIMEOUT_THD_MS (2000)
...
#define PRINT_TASK(element) \
	printk(KERN_INFO "[%s:%d]: PRINT_TASK id: %ld, type: %x, instAddr: %lx, prior: %d\n", __func__, __LINE__, element.taskId, element.taskType, element.offchipInstAddr, element.prior)
...
static const struct ipu_ioctl_map ipu_ioctl_maps[] = {
	{RDCONFIG_DWORD,			FUNC_AND_NAME(ipu_read_dword)},
	{WRCONFIG_DWORD,			FUNC_AND_NAME(ipu_write_dword)},
	{SETCONFIG_MAP,				FUNC_AND_NAME(ipu_set_map)},
	{SETCONFIG_UNMAP,			FUNC_AND_NAME(ipu_set_unmap)},
	{SETCONFIG_RESET_VIRT_ADDR,	FUNC_AND_NAME(ipu_set_reset_virt_addr)},
	{SETCONFIG_RESET_STATISTIC,	FUNC_AND_NAME(ipu_set_reset_statistic)},
	{SETCONFIG_REPORT_STATISTIC,FUNC_AND_NAME(ipu_set_report_statistic)},
	{SETCONFIG_UPDATE_PTE,		FUNC_AND_NAME(ipu_set_update_pte)},
	{IN_TASKQUEUE,				FUNC_AND_NAME(ipu_process_workqueue)},
...
int set_offchip_inst_addr(unsigned long addr) {
	...
	printk(KERN_INFO"[%s]: addr is %lx ++++\n", __func__, addr);

	/* write offchip inst addr to instram */
	inst_addr = (uint32_t) (addr + 384);
...
static int start_ipu(void)
{
	...
	if(IPU_TO_START!= ioread32((void *)((unsigned long)adapter->config_reg_virt_addr + IPU_START_REG))) {
		...
		if (kfifo_peek(&FIFO_TaskElements, &head)) {
			...
			printk(KERN_DEBUG"[%s]: START_COMPUTE_1, offchipInstAddr=0x%lx\n", __func__, head.offchipInstAddr);
			if (0 == ret) {
				/* update pte */
				ipu_smmu_pte_update();
				...
				printk(KERN_DEBUG"[%s]: START_COMPUTE_2, offchipInstAddr=0x%lx\n", __func__, head.offchipInstAddr);
				...
				udelay(3);
				printk(KERN_DEBUG"[%s]: START_COMPUTE_3, offchipInstAddr=0x%lx\n", __func__, head.offchipInstAddr);
...
static void start_ipu_workqueue(void)
{
	...
	printk(KERN_DEBUG"[%s]: start_ipu done!\n", __func__);
}
...
bool ipu_get_feature_tree (struct device *dev)
{
	...
	if (strncmp(str, "kirin970_cs", sizeof("kirin970_cs")) == 0) {
		...
		adapter->feature_tree.performance_monitor = true;  // for DEBUG
	} else {
...
{
#ifdef CONFIG_HISI_IPU_REGULATOR
	int ret;
...
void ipu_reset_proc(unsigned int addr)
{
	...
	// IOREAD_RANGE(adapter->ics_irq_io_addr, 0xc);
...
static int ipu_reset(void *arg)
{
	...
	while (1) {
		...
		if (!kfifo_is_empty(&FIFO_TaskElements)) {
			/* handle compute task */
			printk(KERN_DEBUG"[%s]: handle compute task", __func__);
			start_ipu_workqueue();
		} else {
...
static int ipu_open(struct inode *inode, struct file *filp)
{
	...
	/* set IPU clock to 900M */
	iowrite32(0xF0001000, (void *)((unsigned long)adapter->peri_io_addr + adapter->peri_reg_offset.clkdiv8));

	/* clk ics gt */
	iowrite32(0x40004000, (void *)((unsigned long)adapter->peri_io_addr + adapter->peri_reg_offset.clkdiv18));

	/* select clk div of 1 */
	iowrite32(0x7E000000, (void *)((unsigned long)adapter->peri_io_addr + 0xE4));

	/* clk ics gt in media2 CRG */
	iowrite32(0x00000004, (void *)((unsigned long)adapter->media2_io_addr + adapter->media2_reg_offset.peren0));

	/* set VCODECBUS to 450M */
	iowrite32(0x003F0001, (void *)((unsigned long)adapter->peri_io_addr + 0xBC));
	iowrite32(0x000F0001, (void *)((unsigned long)adapter->peri_io_addr + adapter->peri_reg_offset.clkdiv8));
	...
	ipu_reset_proc((unsigned int)adapter->reset_va); // reset when ipu device open
...
static long ipu_read_dword(struct cambricon_ipu_private *dev, unsigned long arg, struct ioctl_out_params *out_params)
{
	...
	printk(KERN_ERR"[%s]: Read CONFIG REG dword offset 0x%p, value is 0x%x\n",
...
static ioctl_cb ipu_obtain_ioctl_callback(unsigned int cmd)
{
	...
	for (cnt = 0; cnt < size; cnt++) {
		if (ipu_ioctl_maps[cnt].cmd == cmd) {
			printk(KERN_INFO "[%s], cmd %x, func %s\n", __func__, cmd, ipu_ioctl_maps[cnt].name);
...
static long ipu_process_workqueue(struct cambricon_ipu_private *dev, unsigned long arg, struct ioctl_out_params *out_params)
{
	...
	if (!copy_from_user(&element, (void __user *)arg, sizeof(element))) {
		...
		printk(KERN_INFO "task element info, id: %ld, type: %x, instAddr: %lx, prior: %d\n", element.taskId, element.taskType, element.offchipInstAddr, element.prior);
...
static int cambricon_ipu_probe(struct platform_device *pdev)
{
	...
	if (res_cfg->start != 0xff400000) {
		res_cfg->start = 0xff400000;
	}
