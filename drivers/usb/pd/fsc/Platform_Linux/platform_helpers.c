...
static struct wake_lock pd_wake_lock;
...
static void pd_op_wake_lock(void)
{
    if (!wake_lock_active(&pd_wake_lock)) {
        wake_lock(&pd_wake_lock);
        pr_err("pd wake lock\n");
    }
}
static void pd_op_wake_unlock(void)
{
    if (wake_lock_active(&pd_wake_lock)) {
        wake_unlock(&pd_wake_lock);
        pr_err("pd wake unlock\n");
    }
}
...
FSC_S32 fusb_EnableInterrupts(void)
{
	...
	wake_lock_init(&pd_wake_lock, WAKE_LOCK_SUSPEND, "pd_wakelock");
    ...
    ret = devm_request_threaded_irq(&chip->client->dev, chip->gpio_IntN_irq, NULL, _fusb_isr_intn,
                  IRQF_NO_SUSPEND |IRQF_ONESHOT | IRQF_TRIGGER_FALLING, FUSB_DT_INTERRUPT_INTN, chip);  // devm_* allocation/free handled by system
    if (ret)
    {
