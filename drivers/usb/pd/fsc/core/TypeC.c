...
CCTermType              VCONNTerm;			// Active VCONN termination value
...
void InitializeTypeCVariables(void)
{
    ...
    SourceCurrent = utccDefault;                                                   // Set 1.5A host current
    updateSourceCurrent();
...
void StateMachineTypeC(void)
{
	...
	pr_info("FUSB  %s --- finishing Type-C State Machine!  TCState: %d, CC1: %d, CC2: %d \n", __func__, ConnState, blnCCPinIsCC1 == TRUE ? 1 : 0, blnCCPinIsCC2 == TRUE ? 1 : 0);
#endif // FSC_INTERRUPT_TRIGGERED
}
    ...
    if (Registers.Status.I_TOGDONE)
    {
        ...
        switch (Registers.Status.TOGSS)
        {
            ...
            case 0b111: // Ra detected on both CC1 and CC2
                ...
                {
                    SetStateAttachWaitAccessory();                        // So go to the AttachWaitSnk state
                }
#endif // FSC_HAVE_SRC
                break;
#endif // FSC_HAVE_SRC
...
void StateMachineAttachWaitSource(void)
{
    ...
    {
    //pr_err("FUSB  %s  CC1 Debounced\n", __func__);
        if(CC1TermCCDebounce != CCTypeUndefined)
        {
            peekCC2Source();
			pr_err("FUSB  %s  PeekCC2 VBUS = %d CC2-Previous = %d CC2-CCDebounce = %d\n", __func__,Registers.Status.VBUSOK,  CC2TermPrevious, CC2TermCCDebounce);
        }
    }
...
void SetStateUnattached(void)
{
    ...
    platform_enable_timer(FALSE);
#endif // FSC_INTERRUPT_TRIGGERED
    UpdateCurrentAdvert(utccDefault);
...
#ifdef FSC_DEBUG
    WriteStateLog(&TypeCStateLog, ConnState, Timer_tms, Timer_S);
pr_err("FUSB  %s  ConnState  = %d VBUS = %d\n", __func__,ConnState,Registers.Status.VBUSOK);
#endif // FSC_DEBUG
}
...
void SetStateAttachWaitSource(void)
{
    ...
    {
        peekCC2Source();
		pr_err("FUSB  %s  ConnState  = %d VBUS = %d CC2-VCONN = %d\n", __func__,ConnState,Registers.Status.VBUSOK, CC2TermPrevious);
    ...
    else
    {
        ...
        peekCC1Source();
		pr_err("FUSB  %s  ConnState  = %d VBUS = %d CC1-VCONN = %d\n", __func__,ConnState,Registers.Status.VBUSOK, CC1TermPrevious);
        if(Registers.DeviceID.VERSION_ID == VERSION_302B)
        {
...
void SetStateAttachedSource(void)
{
    ...
    UpdateCurrentAdvert(utcc1p5A);                                              // 1.5A for PD  
...
{
pr_err("FUSB  %s  got here ... so VBUS should be enabled next after role swap", __func__);
    platform_set_vbus_lvl_enable(VBUS_LVL_5V, TRUE, TRUE);          // Enable only the 5V output
    ConnState = AttachedSource;                                     // Set the state machine variable to Attached.Src
    TypeCSubState = 0;
    sourceOrSink = SOURCE;
...
void debounceCC(void)
{
    ...
    if (Registers.Switches.MEAS_CC1)                                            // If we are looking at CC1
    {
		...
		//ToggleMeasureCC2();					// toggle measure to VCONN
    ...
    else if(Registers.Switches.MEAS_CC2)                                        // Otherwise we are looking at CC2
    {
		...
		//ToggleMeasureCC1();					// toggle measure to VCONN
		//VCONNTerm = DecodeCCTermination();
    }
