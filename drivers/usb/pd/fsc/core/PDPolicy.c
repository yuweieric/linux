...
void InitializePDPolicyVariables(void)
{
    ...
    SinkRequestMaxPower = 36000;                                                // Maximum power the sink will request (36000 * 0.5mW = 18W) (0.5W, used to calculate current as well)
    SinkRequestOpPower = 36000;                                                 // Operating power the sink will request (0.5W, sed to calculate current as well)
    ...
    //第一组和第二组中的值除了前两项外，其他值均已第一组为准，第二组中的其他值设为0
    //low power
    CapsSink[0].FPDOSink.Voltage = 100;                                         // Set 5V for the first supply option
    CapsSink[0].FPDOSink.OperationalCurrent = 200;                              // Set that our device will consume 2000mA for this object
    ...
    CapsSource[0].FPDOSupply.DataRoleSwap = TRUE;                               // By default, enable DR_SWAP
    ...
    CapsSource[0].FPDOSupply.USBSuspendSupport = FALSE;                         // By default, disable USB Suspend
...
void USBPDDisable(FSC_BOOL DeviceUpdate)
{
    ...
    platform_notify_pd_contract(FALSE, 0, 0);
    ...
    ProtocolFlushTxFIFO();
        ...
        Registers.Mask.M_ACTIVITY = 1;// Mask PD Interrupts
...
//    }
    ...
    DeviceWrite(regSwitches1, 1, &Registers.Switches.byte[1]);            break;
        default:
            PolicySubIndex = 0;
            break;
    }
...
{
	platform_notify_pd_state(SOURCE);
...
void PolicySourceEvaluatePRSwap(void)
{
    ...
    switch(PolicySubIndex)
    {
        case 0: // Send either the Accept or Reject command
			// if check will cause Ellisys test TD.PD.VNDI.E5 to fail  --  Version 3.1.5989
            ...
            else
            {
                if (PolicySendCommand(CMTAccept, peSourceEvaluatePRSwap, 1) == STAT_SUCCESS) // Send the Accept message and wait for the good CRC
                {
                    ...
                    platform_notify_pd_contract(FALSE, 0, 0);
                    RoleSwapToAttachedSink();
                    PolicyStateTimer = tSrcTransition - 2;
                }
            }
            break;
        case 1:
        ...
        case 2:
            ...
            {
                //RoleSwapToAttachedSink();
                PolicyStateTimer = 0;                              // Allow some extra time for VBUS to discharge
                PolicyIsSource = FALSE;
                Registers.Switches.POWERROLE = PolicyIsSource;
                DeviceWrite(regSwitches1, 1, &Registers.Switches.byte[1]);
                PolicySubIndex++;
...
void PolicySinkEvaluateCaps(void)
{
    ...
    {
        PartnerCaps.object = CapsReceived[reqPos-1].object;
...
void PolicySinkSelectCapability(void)
{
    switch (PolicySubIndex)
    {
        case 0:
            ...
            {
                NoResponseTimer = tSenderResponse - 2;                                   // If there is a good CRC start retry timer
            }
            break;
       case 1:
            if (ProtocolMsgRx)
...
void PolicySinkReady(void)
{
    ...
    if (ProtocolMsgRx)                                                          // Have we received a message from the source?
    {
        ...
        if (PolicyRxHeader.NumDataObjects == 0)                                 // If we have received a command
        {
            switch (PolicyRxHeader.MessageType)                                 // Determine which command was received
            {
                ...
                case CMTSoftReset:
                    ...
                    break;
                default:                                                        // For all other commands received, simply ignore them
			pr_err("FUSB  %s  Error: Invalid Command Message \n", __func__);
			PolicySubIndex = 0;                                         // Reset the sub index
                    PDTxStatus = txIdle;                                        // Reset the transmitter status
                    break;
            }
        ...
        else
        {
			...
			pr_err("FUSB  %s  Error: Invalid Data Message \n", __func__);
			PolicySubIndex = 0;                                                     // Reset the sub index
                    PDTxStatus = txIdle;                                                    // Reset the transmitter status
                    break;
            }
...
void PolicySinkEvaluatePRSwap(void)
{
    ...
    {
        case 0: // Send either the Accept or Reject command
			// if check will cause Ellisys test TD.PD.VNDI.E6 to fail  --  Version 3.1.5989
            if (((PartnerCaps.FPDOSupply.SupplyType == pdoTypeFixed) && (PartnerCaps.FPDOSupply.DualRolePower == FALSE))) //|| // Determine Accept/Reject based on partner's Dual Role Power
                    ...
                    //((CapsSink[0].FPDOSink.ExternallyPowered == FALSE) && (PartnerCaps.FPDOSupply.ExternallyPowered == TRUE)))  // Must also reject if partner is externally powered and we are not
            {
            ...
            else
            {
                ...
                {
                pr_err("FUSB  --Accept Completed %s \n", __func__);
                    IsPRSwap = TRUE;
                    PolicyHasContract = FALSE;
                    platform_notify_pd_contract(FALSE, 0, 0);
                    PRSwapTimer = tPRSwapBailout;                               // Initialize the PRSwapTimer to indicate we are in the middle of a swap
        ...
        case 1: // Wait for the PS_RDY command to come in and indicate the source has turned off VBUS
            if (ProtocolMsgRx)                                                  // Have we received a message from the source?
            {
                ...
                if (PolicyRxHeader.NumDataObjects == 0)                         // If we have received a command
                {
                    ...
                    {
                        case CMTPS_RDY:                                         // If we get the PS_RDY message...
						pr_err("FUSB  --PS_RDY received %s \n", __func__);
                            RoleSwapToAttachedSource();                         // Initiate the Type-C state machine for a power role swap
                            PolicyIsSource = TRUE;
                            Registers.Switches.POWERROLE = PolicyIsSource;
                            DeviceWrite(regSwitches1, 1, &Registers.Switches.byte[1]);
                            PolicyStateTimer = 0;                  // Allow the output voltage to rise before sending the PS_RDY message
                            PolicySubIndex++;                                   // Increment the sub-index to move onto the next state
                            break;
                        default:                                                // For all other commands received, simply ignore them
                            break;
...
void processDMTBIST(void)
{
    ...
    switch (bdo)
    {
        ...
        else
        {
            ...
            Registers.MaskAdv.M_HARDRST = 0;
            Registers.MaskAdv.M_GCRCSENT = 1;
            DeviceWrite(regMaska, 2, &Registers.MaskAdv.byte[0]);
