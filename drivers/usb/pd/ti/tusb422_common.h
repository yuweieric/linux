...
#define PRINT(str, args...)  printk("TUSB422 "str, ##args)


#if DEBUG_LEVEL >= 1
    // Variadic macro requires "--gcc" compiler switch.
    #define CRIT(str, args...)  printk("TUSB422 "str, ##args)
#else
...
#if DEBUG_LEVEL >= 2
    // Variadic macro requires "--gcc" compiler switch.
    #define DEBUG(str, args...)  printk("TUSB422 "str, ##args)
#else
...
#if DEBUG_LEVEL >= 3
    // Variadic macro requires "--gcc" compiler switch.
    #define INFO(str, args...)  printk("TUSB422 "str, ##args)
#else
...
#define PDO_OP_PWR(mw)     ((uint16_t)((mw)/250))   // 250mW LSB

typedef enum
{
    FAST_ROLE_NOT_SUPPORTED = 0,
    FAST_ROLE_DEFAULT_PWR,
    FAST_ROLE_1P5_AMP,
    FAST_ROLE_3P0_AMP
} fast_role_t;
